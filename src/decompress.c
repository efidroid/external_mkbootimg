...
int decompress_file(FILE *source, FILE *dest, size_t* lenp)
{
    int ret;
    int i;
    unsigned have;
    ...
    /* read gzip header */
    size_t skip = GZIP_HEADER_LEN;
    fread(in, 1, skip, source);
    if (ferror(source)) {
	...
	if (in[3] & 0x8) {
        /* read filename */
        fread(in, 1, GZIP_FILENAME_LIMIT, source);
        if (ferror(source)) {
            (void)inflateEnd(&strm);
            return Z_ERRNO;
        }
        ...
        skip = GZIP_HEADER_LEN;
		for (i = 0; i < GZIP_FILENAME_LIMIT && *inptr++; i++) {
			skip++;
		}
